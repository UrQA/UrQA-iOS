...
@implementation HQADeviceManager

+ (HQADeviceInfo *)createDeviceReport
{
    HQADeviceManager *manager = [self sharedInstance];
    
    HQADeviceInfo *device = nil;
    if(manager)
    {
        [manager reloadInformation];
        
        device = [[HQADeviceInfo alloc] init];
...
+ (HQADeviceInfo *)createDeviceReportFromCrashReport:(PLCrashReport *)crashReport deviceInfo:(HQADeviceInfo *)deviceInfo
{
    if (![crashReport isKindOfClass:[PLCrashReport class]])
        return nil;
    
    HQADeviceInfo *device = [[HQADeviceInfo alloc] init];
    @try
    {
...
+ (HQADeviceManager *)sharedInstance
{
    static HQADeviceManager *manager = nil;
    
    // It makes singleton object thread-safe
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[HQADeviceManager alloc] init];
