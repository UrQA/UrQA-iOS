...
@interface URQAEventPathManager()
{
...
@implementation URQAEventPathManager

+ (URQAEventPathManager *)sharedInstance
{
    static URQAEventPathManager *manager = nil;

    // It makes singleton object thread-safe
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[URQAEventPathManager alloc] init];
    });
...
{
    URQAEventPath *eventPath = [[URQAEventPath alloc] init];
...
- (NSArray *)arrayData
{
    ...
    for(URQAEventPath *eventPath in _eventPaths)
