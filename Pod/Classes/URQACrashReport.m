...
@interface URQACrashReport(PrivateAPI)
...
@interface URQACrashReport()
{
    NSMutableArray              *_eventPaths;
    NSMutableDictionary              *_URQAData;
    URQADataObjectMerge          *_mergeObject;
}
@end

@implementation URQACrashReport
...
- (void)refreshRequestData
{
    ...
    if (_URQAData)
        [dict setObject:_URQAData forKey:@"URQAData"];

    [_mergeObject setObject2:[[URQADataObject alloc] initWithData:dict]];
}
...
- (id)init
{
    ...
    if(self)
    {
        _mergeObject = [[URQADataObjectMerge alloc] init];

        _deviceInfo = nil;
        _eventPaths = [[NSMutableArray alloc] init];
        _URQAData = [[NSMutableDictionary alloc] init];
        _exceptionName = @"";
        _exceptionReason = @"";
        _exceptionFunction = @"";
...
- (id)initWithCrashReport:(PLCrashReport *)crashReport deviceInfo:(URQADeviceInfo *)deviceInfo exceptionInfo:(URQAExceptionInfo *)exceptionInfo
{
    self = [super init];
    if (self)
    {
        _mergeObject = [[URQADataObjectMerge alloc] init];
        _URQAData = [NSMutableDictionary new];
        // Create Report
        ...
        [_URQAData setObject:report_arch forKey:@"arch"];
        ...
        if (crashReport.exceptionInfo != nil && crashReport.exceptionInfo.stackFrames != nil && [crashReport.exceptionInfo.stackFrames count] > 0) {
            ...
            for (NSUInteger frame_idx = 0; frame_idx < [exception.stackFrames count]; frame_idx++) {
                ...
                [eStackArray addObject:[URQACrashReport formatStackFrame: frameInfo frameIndex: frame_idx report: crashReport lp64: lp64]];
            }
            [report_process setObject:eStackArray forKey:@"exceptionStack"];
        }
        [_URQAData setObject:report_process forKey:@"process"];
        // report_thread
        // thread information
        ...
        for (PLCrashReportThreadInfo *thread in crashReport.threads) {
            ...
            for (NSUInteger frame_idx = 0; frame_idx < [thread.stackFrames count]; frame_idx++) {
                ...
                NSMutableDictionary *t = [URQACrashReport parseStackFrame:frameInfo frameIndex:frame_idx report:crashReport lp64:lp64];
                [frame addObject:t];
            }
            [ti setObject:frame forKey:@"frame"];
            [report_thread addObject:ti];
        }
        [_URQAData setObject:report_thread forKey:@"thread"];
        /* Registers */
        ...
        [_URQAData setObject:report_register forKey:@"register"];
        [self setDeviceInfo:[URQADeviceManager createDeviceReportFromCrashReport:crashReport deviceInfo:deviceInfo]];
        _eventPaths = [[NSMutableArray alloc] init];
...
- (NSDictionary *)URQAData
{
    return [NSDictionary dictionaryWithDictionary:_URQAData];
}

- (void)setURQAData:(NSDictionary *)URQAData
{
    _URQAData = [NSMutableDictionary dictionaryWithDictionary:URQAData];
    [self refreshRequestData];
}
...
@implementation URQACrashReport (PrivateMethods)
